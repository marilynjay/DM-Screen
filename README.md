...
- **App code is one file:** `src/app.jsx` (~950KB). React 18, hooks only, no other runtime dependencies — no lucide, no router, nothing. The Vite scaffold around it (`index.html`, `src/main.jsx`, `vite.config.js`) is the only other code that runs in the browser.
...
3. **Data blocks** — `BESTIARY` (330 monsters, one giant line) and `SPELL_REF` (339 spells, one giant line, keys lowercase). When grepping, filter out these line numbers or your matches drown. `CONDITIONS`, `ITEMS` (loot), `LAIR_THEMES` (18 biomes × 4 lair-action suggestions), `ENCOUNTER_POOLS` (18 biomes of curated monster names), XP tables.
...
2. Split `BESTIARY`, `SPELL_REF`, `LAIR_THEMES`, `ENCOUNTER_POOLS`, `ITEMS` into imported JSON modules.
